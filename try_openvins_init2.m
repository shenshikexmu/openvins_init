...
for n=1:m
    ...
    if n==1
        ...
        for i=1:size(pts{n},1)
            npt_l=undistort_cv(pts{n}(i,1:2)-[1,1], camK,camD);   
            features=update_feature(features, ids{n}(i,1), timestamp, cam_id, pts{n}(i,1), pts{n}(i,2), npt_l(1,1), npt_l(1,2));
        end
        continue;   
    end
